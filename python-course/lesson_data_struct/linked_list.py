...
class LinkedList:
    ...
    def _get_prev(self, node):
        ...
        if node is self.head:         # если указанный узел это голова
            return None              # то преддущего узла нет

        for current in self.nodes(): # итерируемся по узлам списка
    ...
    def nodes(self):
        ...
        while current is not None:  # счетчик ссылается на узел
    ...
    def insert(self, value, node=None):
        """Создает и добавляет после указанного узла и возвращает добавленный узел.
        ...
        if node is None:                        # заменяем голову списка
            ...
            if node.next is None:               # если вставка в конец списка
                self.tail = current             # то текущий узел становится хвостом
            else:
    ...
    def remove(self, node):
        ...
        if self.is_empty():                             # если список пустой, то удалять нечего
            raise ValueError('Linked list is empty.')   # по-питоновски, нужно бросить исключение

        prev = self._get_prev(node)                     # поулчаем ссылку на предыдущий узел
        ...
        if node is self.tail:                           # если удаляемй узел это хвост
    ...
    def remove_tail(self):
        """Удаляет последний узел списка и возвращает данные узла."""
    ...
    def search(self, value):
        ...
        for node in self.nodes():   # итерируемся по узлам списка
            if node.value == value: # если значения узла совпадают с искомым, 
...
if __name__ == '__main__':
    ...
    node3 = ll.insert(2, node2)
    print(node3)

    print(ll)
    print(ll.remove(node1))
    print(ll)
