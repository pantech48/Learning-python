...
from task_diary.helpers import (
	promt, input_int, input_date,
	print_table, print_task, input_datetime
)
from task_diary.services import make_db_connection
from task_diary.decorators import make_db_connection


Action = namedtuple('Action', ('func', 'title'))
actions = {
	'q': Action(sys.exit, "Выйти"),
}
...
def input_task():
	...
	def cb(task_id):
		...
		return task
	return promt('Введите ID задачи', type_cast=cb)
...
def input_task_data(task=None):
	...
	data['title'] = promt('Название', default=task.get('title', ''))
	...
	data['description'] = promt('Описание', default=task.get('description', ''))
