...
todo: Стандартные потоки ввода/вывода
...
Режимы записи:
    ...
    а - append - добавление/дозапись в конец файла 
...
Режимы чтения:
    ...
    w+ - перезапись, с возможностью чтения
    a+ - дозапись, с возможностью чтения
...
# todo: Контексный менеджер
# При работе с файлом всегда используем контекстный менеджер
with open('out.txt') as f, open(__file__):#Текущее имя файла) as lesson:
    print(f.read())
    print(lesson.read())
# В любом случае после окончания работы - файл будет закрыт
