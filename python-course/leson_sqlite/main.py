...
# todo: Установка соединения с сервером БД, выбрать базу данных
...
#     )   
#     '''
#     cursor = conn.cursor() # некий посредник, с помощью которого можно выполнять запрос
#     cursor.execute(sql) # делаем запрос БД    
#     conn.commit() 
# finally:
#     conn.close()
...
with sqlite3.connect('diary.sqlite3', detect_types=sqlite3.PARSE_DECLTYPES) as conn:
    ...
    """
    UPDATE task SET title=?, plannde=?, description=? WHERE id=?
